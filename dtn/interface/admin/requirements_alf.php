...
if($action=="modif"){
		if (isset ($age) && isset($position_id) && isset ($carac_1) && isset ($week) && isset ($level_1)   && isset($carac_2) && isset ($level_2)  ){
			...
			<li> niveau en Carac 1 
			<select name="level_1" size="1" >
                <?php
...
// *** selection des minimas à afficher
...
if (!isset ($subposition)){
	...
	$sql = "select * from ht_requirements where position_id='".$showposition."' and sub_position_id='".$subposition."' order by position_id,age,week ";
...
echo "test<br />";
echo $lstPoste["position_id"]["sub_position_id"]["descriptifPoste"];
echo "<br />fintest";
echo "test<br />";
print_r ($lstPoste);
echo "<br />fintest";
...
<tr>
<td><?=$l["age"]?> </td>
<td> <?=$lstPosition[$l["position_id"]-1]["descriptifPosition"]?></td>
<td><?=$lstPoste["sub_position_id"]["descriptifPoste"]?></td>
<td><?=$l["week"]?></td>

<td><?=$lstCarac[$l["carac_1"]-1]["nomTypeCarac"]?></td><td><?=$lstLevel[$l["level_1"]-1]["intituleCaracFR"]?> </td>
<?php if ($l["carac_2"]==-1 ){ ?>
	<td>-</td><td>-</td>
<?php	
}else{
?>
	<td><?=$lstCarac[$l["carac_2"]-1]["nomTypeCarac"]?></td><td><?=$lstLevel[$l["level_2"]-1]["intituleCaracFR"]?> </td>
<?php } ?>
<td>--</td>
