...
require_once("includes/serviceDTN.php");
...
case "reactivAdmin":
	...
	$sql = $conn->exec("UPDATE ht_admin SET affAdmin=1 WHERE idAdmin=".$id."");
...
case "modifClub";
	...
	majDB($tbl_clubs,$id);
	header("location: clubs/index.php?msg=Modification du club OK");
...
case "modifJoueur";
	...
	$req = $conn->query($sql);
	$infJ = $req->fetch(PDO::FETCH_ASSOC);
	...
	header("location: joueurs/checkPlayer.php?msg=Le joueur a bien ete modifie");
	break;
...
case "modifPays";
	majDB($tbl_pays,$id);
	header("location: pays/index.php?msg=Le pays a bien ete modifie");
	break;
...
case "clotureRapport":
	$sql = $conn->exec("update $tbl_config set valeurConfig  = \"0\" where nomConfig = \"rapport\"");
	header("location: index2.php");

	break;

case "ouvrirRapport":
	$sql = $conn->exec("update $tbl_config set valeurConfig  = \"1\" where nomConfig = \"rapport\"");
	header("location: index2.php");
...
case "supprMatch":
	$sql = $conn->exec("update $tbl_perf set affPerf = 0 where idPerf = $idPerf");
...
case "archiveJoueur":
	...
	$sql = $conn->exec(" UPDATE ht_joueurs SET archiveJoueur = 1, dtnSuiviJoueur_fk  =0, affJoueur = 0 WHERE idJoueur = $id") or die ("Erreur :$sql");
...
case "joueurSupprimeDTN":
	...
	$sql = $conn->exec(" UPDATE ht_joueurs SET  dtnSuiviJoueur_fk  =0 WHERE idJoueur = '$idJoueur'") or die ("Erreur :$sql");
...
case "joueurSupprimeSecteur":
	...
	$sql = $conn->exec(" UPDATE ht_joueurs SET  dtnSuiviJoueur_fk  =0, ht_posteAssigne=0 WHERE idJoueur = '$idJoueur'") or die ("Erreur :$sql");
...
case "desarchiveJoueur":
	...
	$sql = $conn->exec(" UPDATE ht_joueurs SET archiveJoueur = 0, dtnSuiviJoueur_fk  =0, affJoueur = 1 WHERE idJoueur = $id") or die ("Erreur :$sql");
...
case "addNiveau":
	...
	if (  ($niveau==0 &&( $champ=='idEndurance' || $champ=='idGardien' ||$champ=='idConstruction' ||$champ=='idPasse' ||$champ=='idAilier' ||$champ=='idDefense' ||$champ=='idButeur' ||$champ=='idPA'  ||$champ=='idExperience_fk'   ))
		...
		$sql = $conn->exec("update $tbl_joueurs set $champ = $niveau where idjoueur = $idJoueur");
